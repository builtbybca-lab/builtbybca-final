        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} onClick={() => setSelectedCategory(category)} className={`${selectedCategory === category ? "bg-bca-red text-white hover:bg-bca-red-hover" : "border-white/20 text-white hover:bg-bca-red/20 hover:border-bca-red"}`}>
                  {category}
                </Button>)}
            </div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-bca-dark-card border-white/20">
          {selectedEvent && <div className="space-y-6">
              <div className="relative">
                <img src={selectedEvent.image_url} alt={selectedEvent.title} className="w-full h-64 object-cover rounded-lg" />
              ...
              {selectedEvent.speakers && selectedEvent.speakers.length > 0 && <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {selectedEvent.speakers.map((speaker, index) => <div key={index} className="bg-bca-dark/50 p-4 rounded-lg border border-white/10">
